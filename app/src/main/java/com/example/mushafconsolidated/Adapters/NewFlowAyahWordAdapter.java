...
import com.example.mushafconsolidated.fragments.NewWordMorphologyDetails;
...
public class NewFlowAyahWordAdapter extends RecyclerView.Adapter<NewFlowAyahWordAdapter.ItemViewAdapter>
        //implements OnItemClickListenerOnLong {
{
    ...
    private final LinkedHashMap<Integer, ArrayList<QuranCorpusWbw>> ayahWordArrayList;
  ...
  //MaterialTextView arabic;
   ...
   // private CorpusWbwWord ayahWord;
    ArrayList<QuranCorpusWbw> ayahWord;
    private boolean isaudio;

    public NewFlowAyahWordAdapter(boolean isaudio, LinkedHashMap<Integer, ArrayList<CorpusWbwWord>> passage, ArrayList<MafoolMutlaqEnt> mutlaqent, ArrayList<TameezEnt> tameezEnts, ArrayList<BadalErabNotesEnt> badalErabNotesEnt, ArrayList<LiajlihiEnt> liajlihient, ArrayList<HalEnt> jumlahaliya, ArrayList<MafoolBihi> mafoolBihis, ArrayList<String> header, List<QuranEntity> allofQuran,
                                  LinkedHashMap<Integer, ArrayList<QuranCorpusWbw>> ayahWordArrayList, Context context, long surah_id, String surahName, int ismakki, OnItemClickListenerOnLong listener) {
    ...
    public long getItemId(int position) {
        ...
        long ayah = 0;
  ayah = ayahWordArrayList.get(position).get(0).getCorpus().getAyah();
    ...
    //    return ayahWordArrayList.get(position).getWord().get(position);
        return ayahWordArrayList.get(position).get(0).getCorpus();
...
//bg_black
    ...
    private void displayAyats(boolean showrootkey, NewFlowAyahWordAdapter.ItemViewAdapter holder, int position, SharedPreferences sharedPreferences, Typeface custom_font, boolean showErab, boolean showWordColor, boolean showTransliteration, boolean showJalalayn, boolean showTranslation, boolean showWordByword, String whichtranslation, boolean showKathir) {
        ...
        ArrayList<QuranCorpusWbw> quranCorpusWbws = ayahWordArrayList.get(position);
        if (null != entity) {
            storepreferences(entity);
        }
        SpannableString quranverses = ayahWordArrayList.get(position).get(0).getSpannableverse();
        //   if (!isaudio){
        ...
        wordBywordWithTranslation(showrootkey, holder, custom_font, showWordColor, wbw, ayahWordArrayList, showWordByword,position);
    ...
    private void wordBywordWithTranslation(boolean showrootkey, ItemViewAdapter holder, Typeface custom_font, boolean showWordColor, String wbw, LinkedHashMap<Integer, ArrayList<QuranCorpusWbw>> ayahWord, boolean showWbwTranslation, int position) {
        final LayoutInflater inflater =
                (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        holder.flow_word_by_word.removeAllViews();
        ArrayList<QuranCorpusWbw> quranCorpusWbws = ayahWord.get(position);
        for (final QuranCorpusWbw word : quranCorpusWbws) {
            ...
            sb.append(word.getCorpus().getWordno());
         String rootwords =word.getCorpus().getRootaraone()+word.getCorpus().getRootaratwo()+word.getCorpus().getRootarathree()+word.getCorpus().getRootarafour()+word.getCorpus().getRootarafive();
            String arabicword =word.getCorpus().getAraone()+word.getCorpus().getAratwo()+word.getCorpus().getArathree()+word.getCorpus().getArafour()+word.getCorpus().getArafive();

            if (showrootkey) {
                if (rootwords!=null) {
                    spannedroot = getSpannedRoots(word,rootwords);
                } else {
                    spannedroot = SpannableString.valueOf(rootwords);
                }
            ...
            if (showWordColor) {
                ...
                arabicChipview.setText(arabicword);
            ...
            if (showWbwTranslation) {
                switch (wbw) {
                    case "en":
                        translation.setText(word.getWbw().getEn());
                        translation.setPaintFlags(translation.getPaintFlags() | Paint.UNDERLINE_TEXT_FLAG);
                        break;
                    case "bn":
                        translation.setText(word.getWbw().getBn());
                        translation.setPaintFlags(translation.getPaintFlags() | Paint.UNDERLINE_TEXT_FLAG);
                        break;
                    case "in":
                        translation.setText(word.getWbw().getIn());
                        translation.setPaintFlags(translation.getPaintFlags() | Paint.UNDERLINE_TEXT_FLAG);
                        break;
                    case "ur":
                        translation.setText(word.getWbw().getUr());
                        translation.setPaintFlags(translation.getPaintFlags() | Paint.UNDERLINE_TEXT_FLAG);
                        break;
                }
            ...
            if(!showWbwTranslation) {
                ...
                arabicChipview.setOnLongClickListener(new OnLongClickListener() {
                    ...
                    public boolean onLongClick(View v) {
                        ...
                        ArrayList<VerbCorpus> verbCorpusRootWords = utils.getQuranRoot(word.getCorpus().getSurah(), word.getCorpus().getSurah(), word.getCorpus().getWordno());
                        if (verbCorpusRootWords.size() > 0 && verbCorpusRootWords.get(0).getTag().equals("V")) {
                            //    vbdetail = ams.getVerbDetails();
                            System.out.print("check");
                        }
                        ArrayList<NounCorpus> corpusNounWord = utils.getQuranNouns(word.getCorpus().getSurah(), word.getCorpus().getSurah(), word.getCorpus().getWordno());
                        ArrayList<VerbCorpus> verbCorpusRootWord = utils.getQuranRoot(word.getCorpus().getSurah(), word.getCorpus().getSurah(), word.getCorpus().getWordno());
                        NewWordMorphologyDetails qm = new NewWordMorphologyDetails(word, corpusNounWord, verbCorpusRootWord);
    ...
    private void LoadItemList(Bundle dataBundle, QuranCorpusWbw word) {
        if (issentence) {
            SentenceAnalysisBottomSheet item = new SentenceAnalysisBottomSheet();
            item.setArguments(dataBundle);
            String[] data = {String.valueOf(word.getCorpus().getSurah()), String.valueOf(word.getCorpus().getSurah()), word.getWbw().getEn(), String.valueOf((word.getCorpus().getWordno()))};
    ...
    private SpannableString getSpannedRoots(QuranCorpusWbw tag,String rootword) {
   
        return CorpusUtilityorig.ColorizeRootword(tag.getCorpus().getTagone(), tag.getCorpus().getTagtwo(), tag.getCorpus().getTagthree(), tag.getCorpus().getTagfour(), tag.getCorpus().getTagfive(),
                rootword);
    }

    private SpannableString getSpannedWords(QuranCorpusWbw tag) {
     
        return CorpusUtilityorig.NewSetWordSpan(tag.getCorpus().getTagone(), tag.getCorpus().getTagtwo(), tag.getCorpus().getTagthree(), tag.getCorpus().getTagfour(), tag.getCorpus().getTagfive(),
                tag.getCorpus().getAraone(), tag.getCorpus().getAratwo(), tag.getCorpus().getArathree(), tag.getCorpus().getArafour(), tag.getCorpus().getArafive());
    }
    ...
    private void setChapterInfo(ItemViewAdapter holder, ArrayList<QuranCorpusWbw> verse) {
        StringBuilder surahInfo = new StringBuilder();
//        surahInfo.append(surahName+".");
      
        surahInfo.append(verse.get(0).getCorpus().getSurah()).append(".");
        surahInfo.append(verse.get(0).getCorpus().getAyah()).append("-");
        surahInfo.append(SurahName);
        SharedPreferences sharedPreferences =
                androidx.preference.PreferenceManager.getDefaultSharedPreferences(context);
                ...
                fabmenu.setOnClickListener(new View.OnClickListener() {
                    ...
                    private void showFABMenu() {
                        ...
                        tafsir.setOnClickListener(view12 -> {
                            ...
                            int chapter_no = ayahWord.get(0).getCorpus().getSurah();
                            //   int verse = ayahWord.getWord().get(0).getVerseId();
                            int verse = ayahWordArrayList.get(getPosition() - 1).get(0).getCorpus().getAyah() ;
                    ...
                    private void bookMarkSelected(int position) {
                        ...
                        int chapter_no = ayahWord.get(0).getCorpus().getAyah();
                        //   int verse = ayahWord.getWord().get(0).getVerseId();
                        int verse = ayahWordArrayList.get(getPosition() - 1).get(0).getCorpus().getAyah();
